...
export async function GET(request: NextRequest) {
  ...
  // Preview deployments intentionally do not carry production Supabase env vars.
  // Release QA may supply the public project URL + publishable key transiently in this request.
  const suppliedUrl = request.nextUrl.searchParams.get('supabase_url');
  const suppliedKey = request.nextUrl.searchParams.get('supabase_key');
  if (suppliedUrl && suppliedKey) {
    process.env.SUPABASE_URL = suppliedUrl;
    process.env.SUPABASE_PUBLISHABLE_KEY = suppliedKey;
  }
  ...
  const preregistrationUnknownCityGuard = await invoke(preregisterPost, origin, '/api/preregister', {
    first_name: 'PlanetHike QA Unknown City', email: `bad-city.${qaEmail}`, country_code: 'DE', city: 'Definitely Not In The List', interest: 'walker', privacy_accepted: 'on',
  });
  const preregistrationCustomCity = await invoke(preregisterPost, origin, '/api/preregister', {
    first_name: 'PlanetHike QA Custom City', email: `custom.${qaEmail}`, country_code: 'DE', city: 'City not yet listed', custom_city: 'Siegburg', interest: 'walker', privacy_accepted: 'on',
  });
  ...
  const contactTopicGuard = await invoke(contactPost, origin, '/api/contact', {
    name: 'PlanetHike QA Bad Topic', email: `contact-topic.${qaEmail}`, topic: 'not-a-real-topic', message: 'Automated validation test for the contact topic whitelist.', privacy_accepted: 'on',
  });
  ...
  return NextResponse.json({
    ...
    qaEmail,
    tests: { preregistration, preregistrationPrivacyGuard, preregistrationUnknownCityGuard, preregistrationCustomCity, cityLead, cityLeadSafetyGuard, contact, contactPrivacyGuard, contactTopicGuard },
